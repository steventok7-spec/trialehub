    ...
    <div class="bg-white rounded-3xl shadow-sm border border-zinc-100 overflow-hidden min-h-[50vh]">
      @if (loading()) {
        <div class="flex flex-col items-center justify-center h-full py-32">
          ...
          <p class="text-zinc-500 font-medium">Loading expense claims...</p>
        </div>
        ...
        <div class="p-6 space-y-4">
          <div class="flex items-center justify-between mb-6">
            <h3 class="font-bold text-lg text-zinc-900">Expense Claims</h3>
              ...
              <div class="flex justify-between items-start gap-4">
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-sm text-zinc-500 mt-1">
                    {{ req.created_at?.split('T')[0] || 'No date' }}
                  </p>
                  @if (req.reason) {
                    <p class="text-sm text-zinc-600 mt-2 bg-zinc-50 p-3 rounded-lg">{{ req.reason }}</p>
                  }
                  @if (req.amount) {
                    <p class="text-lg font-bold text-zinc-900 mt-2">{{ req.amount | currency:'IDR':'symbol':'1.0-0' }}</p>
                  }
                </div>
...
export class AdminClaims implements OnInit {
  ...
  loadRequests() {
    ...
    this.api.getRequests('claim').subscribe({
      next: (data) => {
        this.requests.set(data);
        this.loading.set(false);
      },
      error: () => {
        this.toast.error('Failed to load expense claims');
        this.loading.set(false);
      }
    });
  }
